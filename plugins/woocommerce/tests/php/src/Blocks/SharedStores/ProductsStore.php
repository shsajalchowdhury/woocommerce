...
class ProductsStore extends \WC_Unit_Test_Case {
	...
	public function test_register_getters_is_idempotent(): void {
		...
		$state = wp_interactivity_state( $this->store_namespace );
		$this->assertArrayHasKey( 'parentProductInContext', $state );
		$this->assertArrayHasKey( 'productVariationInContext', $state );
		$this->assertArrayHasKey( 'productInContext', $state );
		$this->assertInstanceOf( \Closure::class, $state['parentProductInContext'] );
	...
	/**
	 * @testdox state.parentProductInContext resolves to the hydrated product matching state.productId.
	 */
	public function test_parent_product_in_context_getter_reads_from_state(): void {
		...
		$state   = wp_interactivity_state( $this->store_namespace );
		$closure = $state['parentProductInContext'];
	 ...
	 */
	public function test_product_variation_in_context_getter_reads_from_state(): void {
		$this->setExpectedIncorrectUsage( 'WP_Interactivity_API::get_context' );
